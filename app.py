"""
Wine Quality Prediction - Streamlit Deployment App
----------------------------------------------------
Loads the trained Decision Tree model (tuned via GridSearchCV), the
StandardScaler, and the feature list produced in the training notebook,
then lets a user enter wine chemistry values and get a GOOD / BAD
quality prediction.
...
Deploy for free on Streamlit Community Cloud:
    1. Push this folder (app.py, requirements.txt, and the 3 .pkl files) to a GitHub repo.
    2. Go to https://share.streamlit.io , sign in with GitHub.
    3. Click "New app", pick the repo/branch, set main file to app.py, and deploy.
"""
...
try:
    ...
    st.error(
        "Model files not found. Make sure `wine_quality_model.pkl`, "
        "`wine_quality_scaler.pkl`, and `wine_quality_features.pkl` are in the "
        "same folder as app.py (generated by the training notebook)."
    )
    st.stop()
...
# Reasonable default values taken from the dataset's typical ranges
defaults = {
    "fixed acidity": 8.3,
    ...
    "residual sugar": 2.5,
    "chlorides": 0.087,
    "free sulfur dioxide": 15.9,
    "total sulfur dioxide": 46.5,
    "density": 0.9967,
    "pH": 3.31,
    "sulphates": 0.66,
    "alcohol": 10.4,
...
slider_ranges = {
    ...
    "volatile acidity": (0.05, 1.6, 0.01),
...
st.divider()
st.caption(
    "Model: Decision Tree Classifier, tuned with GridSearchCV. "
    "Trained on the UCI Wine Quality (red wine) dataset."
)
